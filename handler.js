...
const ownerSet = new Set(config.owners.map((n) => n + '@s.whatsapp.net'));
const devSet = new Set(config.devs.map((n) => n + '@s.whatsapp.net'));
const getFiles = (dir) => {
    let results = [];
    ...
    try {
        ...
        if (m.isGroup && (cmd.isAdmin || cmd.isBotAdmin || cmd.isGroup !== undefined)) {
            ...
            if (cached && Date.now() - cached.ts < 5 * 60 * 1000) {
                ...
                if (groupMetadata)
                    global.groupMetaCache.set(m.chat, { data: groupMetadata, ts: Date.now() });
            }
            groupName = groupMetadata?.subject || '';
            groupAdmins =
                groupMetadata?.participants?.filter((p) => p.admin === 'admin' || p.admin === 'superadmin') || [];
            ...
            isBotAdmin =
                groupMetadata?.participants?.some((p) => {
                    ...
                    return ((pid === botRaw || plid === botRaw || pphone === botRaw) &&
            ...
            isAdmins =
                groupMetadata?.participants?.some((p) => {
                    ...
                    const plid = p.lid?.split('@')[0]?.split(':')[0];
                    const pphone = p.phoneNumber?.split('@')[0];
                    return ((pid === senderRaw || plid === senderRaw || pphone === senderRaw) &&
                        (p.admin === 'admin' || p.admin === 'superadmin'));
                }) || false;
        }
        ...
        console.log(border('╰───────────────────────────────────────╯'));
        if (cmd.isOwner && !ownerSet.has(senderJid))
            return m.reply('Dueño solamente.');
        if (cmd.isDev && !devSet.has(senderJid))
