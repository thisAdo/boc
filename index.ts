...
const { default: makeWASocket, useMultiFileAuthState, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, jidDecode, Browsers, } = baileys;
...
const groupMetaCache = global.groupMetaCache || (global.groupMetaCache = new Map());
const c = {
...
function cleanCache() {
    try {
        ...
        if (fs.existsSync(tmpFolder)) {
            ...
            if (cleaned > 0)
                console.log(c.dim(`[ 🧹 ] Cache: ${cleaned} archivos eliminados`));
        }
        if (fs.existsSync(AUTH_FOLDER)) {
...
async function getGroupMeta(sock, groupId) {
    ...
    if (data)
        groupMetaCache.set(groupId, { data, ts: Date.now() });
    return data;
}
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            if (!kay.message || kay.key?.remoteJid === 'status@broadcast')
                return;
    ...
    sock.ev.on('connection.update', (up) => {
        ...
        if (connection === 'close') {
            ...
            if (retryCount >= MAX_RETRIES) {
                console.log('\n' + BOX_TOP);
                console.log(`${BAR}  ${c.error(`✗  ${MAX_RETRIES} intentos fallidos`)}               ${BAR}`);
