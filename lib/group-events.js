...
async function handleAntilinkEvent({ sock, m, groupJid, meta }) {
  ...
  const senderParticipant = findParticipant(meta, getSenderCandidates(m))
  if (isAdminFlag(senderParticipant?.admin) || senderParticipant?.isAdmin === true || senderParticipant?.isSuperAdmin === true) return false
  ...
  if (!(isAdminFlag(botParticipant?.admin) || botParticipant?.isAdmin === true || botParticipant?.isSuperAdmin === true)) {
    if (state.avisos) {
      await safeSend(sock, groupJid, {
