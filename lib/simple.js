import { jidDecode, proto, extractMessageContent, getContentType, downloadContentFromMessage, } from '@whiskeysockets/baileys';
...
export const smsg = (sock, m) => {
    ...
    if (!m.chat && m.key) {
        ...
        m.isBot =
            ['HSK', 'BAE', 'B1E', '3EB0', 'B24E', 'WA'].some(a => m.id.startsWith(a) && [12, 16, 20, 22, 40].includes(m.id.length)) ||
    ...
    if (m.message) {
        ...
        if (!m.reply) {
            m.reply = async (text, options = {}) => {
                await sock.sendMessage(m.chat, { text: text, mentions: [m.sender] }, { quoted: m, ...options });
            };
        }
    }
    return m;
