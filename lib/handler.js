...
export async function handleMessage(sock, m) {
  ...
  if (plugin.userAdminOnly) {
    ...
    userIsAdmin = isOwner || await isUserAdmin(sock, m)
    if (!userIsAdmin) return await deny(sock, m, getPermissionError(plugin))
  }
  if (plugin.botAdminOnly) {
    if (!isGroup) return await deny(sock, m, '🪴 *_Este comando solo funciona en grupos_*')
    botIsAdmin = await isBotAdmin(sock, m)
    if (!botIsAdmin) return await deny(sock, m, getPermissionError(plugin))
  }
