export default {
  ...
  run: async (sock, m, { args }) => {
    ...
    if (!target) return m.reply('Etiqueta o responde al mensaje del usuario que deseas ascender.')

    const cleanTarget = target.split(':')[0] + '@s.whatsapp.net'
    const botJid = sock.user.id.split(':')[0] + '@s.whatsapp.net'

    if (cleanTarget === botJid) return m.reply('Ya soy administrador del grupo.')
