        ...
        try {
            if (isMedia) {
                ...
                if (messageType === 'imageMessage' || mime.includes('image')) {
                    return sock.sendMessage(m.chat, { image: media, caption: finalText, mentions }, { quoted: null });
                }
                else if (messageType === 'videoMessage' || mime.includes('video')) {
                    return sock.sendMessage(m.chat, { video: media, mimetype: 'video/mp4', caption: finalText, mentions }, { quoted: null });
                }
                else if (messageType === 'audioMessage' || mime.includes('audio')) {
                    return sock.sendMessage(m.chat, { audio: media, mimetype: 'audio/mp4', ptt: true, mentions }, { quoted: null });
                }
